...
def init():
    global result_dir
    result_dir = osp.join(cfg.exp_path, "result", "epoch{}_nmst{}_scoret{}_npointt{}".format(cfg.test_epoch, cfg.TEST_NMS_THRESH, cfg.TEST_SCORE_THRESH, cfg.TEST_NPOINT_THRESH), cfg.split)
    backup_dir = osp.join(result_dir, "backup_files")
...
def test(model, model_fn, data_name, epoch, semantic_only=False):
    ...
    with torch.no_grad():
        ...
        instance_dataset_root = osp.join(cfg.data_root, cfg.dataset, cfg.split + "_gt")
        evaluator = gorilla3d.ScanNetSemanticEvaluator(semantic_dataset_root, logger=logger)
        inst_evaluator = gorilla3d.ScanNetInstanceEvaluator(instance_dataset_root, logger=logger)
        ...
        for i, batch in enumerate(dataloader):
            ...
            if (epoch > cfg.prepare_epochs) and not semantic_only:
                ...
                for start, end in zip(proposals_offset[:-1], proposals_offset[1:]):
                    semantic_label, _ = stats.mode(semantic_pred[proposals_idx[start:end, 1].long()].cpu().numpy())
            ...
            if cfg.save_semantic:
                ...
                np.save(osp.join(result_dir, "semantic", test_scene_name + ".npy"), semantic_np)

            if cfg.save_pt_offsets:
                os.makedirs(osp.join(result_dir, "coords_offsets"), exist_ok=True)
                ...
                np.save(osp.join(result_dir, "coords_offsets", test_scene_name + ".npy"), coords_offsets)
            ...
            if(epoch > cfg.prepare_epochs and cfg.save_instance):
                ...
                for proposal_id in range(nclusters):
                    ...
                    content = "\n".join(content)
                    with open(osp.join(result_dir, "predicted_masks", test_scene_name + "_%03d.txt" % (proposal_id)), "w") as cf:
                        cf.write(content)
...
if __name__ == "__main__":
    ...
    ##### load model
    utils.checkpoint_restore(model, None, None, cfg.exp_path, cfg.config.split("/")[-1][:-5], use_cuda, cfg.test_epoch, dist=False, f=cfg.pretrain)      # resume from the latest epoch, or specify the epoch to restore

    ##### evaluate
    test(model, model_fn, data_name, cfg.test_epoch, cfg.semantic)
