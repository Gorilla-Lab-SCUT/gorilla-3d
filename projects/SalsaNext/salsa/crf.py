...
class CRF(nn.Module):
  def __init__(self,
               ...
               xyz_sigma: float):
    super().__init__()
    self.iter = torch.nn.Parameter(torch.tensor(iter),
                                   requires_grad=False)
    self.lcn_size = torch.nn.Parameter(torch.tensor([height, width]),
                                       requires_grad=False)
    self.xyz_coef = torch.nn.Parameter(torch.tensor(xyz_coef), requires_grad=False).float()
    self.xyz_sigma = torch.nn.Parameter(torch.tensor(xyz_sigma), requires_grad=False).float()
    ...
    self.local_conn_xyz = LocallyConnectedXYZLayer(height, width, xyz_coef, self.nclasses)
