...
class NuscenesSem(Dataset):
    def __init__(self,
        ...
        self.logger.info("Using {} scans for {}".format(len(self.data_infos), self.task))
    ...
    def __getitem__(self, index):
        ...
        lidarseg_labels_filename = os.path.join(self.data_root, lidar_sd_token["filename"])

        point_labels = np.fromfile(lidarseg_labels_filename, dtype=np.uint8).reshape([-1, 1]) # [N, 1]
        point_labels = self.label_mapper(point_labels) # [N, 1]
        points = np.fromfile(os.path.join(self.data_root, lidar_path), dtype=np.float32, count=-1).reshape([-1, 5]) # [N, 5]

        if self.task == "train":
            points[:, :3] = self.pc_transformer(points[:, :3])

        voxel_position, processed_label, grid_ind, labels, processed_xyz = self.processer(points, point_labels)
        xyz = points[:, :3]
        if self.return_ref:
            sig = points[:, 3]
            if len(sig.shape) == 2:
    ...
    def label_name(self):
        ...
        for i in sorted(list(self.learning_map.keys()))[::-1]:
            nuscenes_label_name[self.learning_map[i]] = self.nuscenesyaml["labels_16"][i]
