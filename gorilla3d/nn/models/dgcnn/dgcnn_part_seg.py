...
class DGCNNPartSeg(nn.Module):
    def __init__(self,
                 cfg: dict,
                 seg_num_all: int,
                 aggregation_nodes: Optional[List[List[int]]] = [[3, 64, 64], [64, 64, 64], [64, 64]],
                 mask_channel:  Optional[int]= 64,
                 mlp_nodes: Optional[List[int]] = [256, 256, 128],
                 dropout: Optional[Union[float, List[float]]] = [0.5, 0.5, 0.0]
                 ):
        ...
        aggregation_dim = sum([node_list[-1] for node_list in aggregation_nodes])
        self.media = nn.Sequential(nn.Conv1d(aggregation_dim, emb_dims, kernel_size=1, bias=False),
                                   nn.BatchNorm1d(emb_dims),
                                   nn.LeakyReLU(negative_slope=0.2))

        self.mask_conv = nn.Sequential(nn.Conv1d(num_class, mask_channel, kernel_size=1, bias=False),
                                       nn.BatchNorm1d(mask_channel),
                                       nn.LeakyReLU(negative_slope=0.2))
        ...
        # output
        self.output = nn.Conv1d(mlp_nodes[-1], seg_num_all, kernel_size=1, bias=False)
    ...
    def forward(self,
        ...
        x = torch.cat((x, mask), dim=1) # [batch_size, emb_dim + mask_channel, 1]
        x = x.repeat(1, 1, num_points) # [batch_size, emb_dim + mask_channel, num_points]
