...
class ConcatAttention(nn.Module):
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__()
        self.self_attn = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, **kwargs)
    ...
    def forward(self,
        ...
        padding_features = []
        key_padding_mask = torch.zeros([bs, length]).bool().to(features.device) # [bs, length]
        for batch_idx in range(bs):
            # get padding faeture
            ids = (batch_ids == batch_idx)
            batch_feature = features[ids] # [num_batch, C]
            padding_length = length - ids.sum()
            padding_feature = F.pad(batch_feature, (0, 0, 0, padding_length), "constant", value=0) # [length, C]
        ...
        attn_output = self.self_attn(padding_features, padding_features, padding_features, key_padding_mask)[0] # [length, bs, C_o]
        ...
        for batch_idx in range(bs):
            ...
            batch_output_feature = attn_output[:num_batch, batch_idx, :] # [num_batch, C_o]
