...
def evaluate_matches(matches: Dict,
                     class_labels: List[str],
                     overlaps: np.ndarray=OVERLAPS,
                     min_region_sizes: List[float]=[MIN_REGION_SIZES[0]],
...
def compute_averages(aps: np.ndarray,
                     class_labels: List[str],
                     overlaps: np.ndarray=OVERLAPS) -> Dict:
    d_inf = 0
    ...
    for (li, label_name) in enumerate(class_labels):
        ...
        avg_dict["classes"][label_name]["ap"] = \
            np.average(aps[d_inf, li, oAllBut25])
        avg_dict["classes"][label_name]["ap50%"] = \
            np.average(aps[d_inf, li, o50])
        avg_dict["classes"][label_name]["ap25%"] = \
            np.average(aps[d_inf, li, o25])
    return avg_dict


def assign_instances_for_scan(scene_name: str,
                              pred_info: Dict,
                              gt_file: str,
                              valid_class_ids: np.ndarray,
                              class_labels: np.ndarray,
                              id_to_label: Dict,
                              min_region_sizes: np.ndarray=MIN_REGION_SIZES):
    try:
